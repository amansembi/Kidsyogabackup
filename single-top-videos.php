...
?>
<div class="container">
	<div class="singlpstttl <?php if(empty($userid)){ echo 'openpoup'; }else{ echo ''; }  ?>">
		...
		<?php global $post;
			...
			$args = array( 'post_type' => 'dgv-upload', 'posts_per_page' => -1);
			   ...
			   $videotop = array_key_first($topviewsVideos);
			   $vd_id = array_key_first($topviewsVideos);
	...
	<div class="container-fluid p-0">
		<div class="videofullsec">
			<div class="showvideo">
				...
				<iframe id="PLAYER1" class="embed_video" src="https://player.vimeo.com/video/<?php echo $vd_id; ?>?autoplay=0&loop=0&controls=0" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>	
			...
			<div class="sglvidfavlistcol">
				...
				$count = 1; 
				foreach($topviewsVideos as $video=>$value){ 
              ...
              ld_video_players[element_id].on('timeupdate', function(something) {				  
				...
				if(autopalystatus1 === true){
					...
					if(idExist == true){
						index = index +1;	
							setTimeout( function(){ 
							 window.location = 'http://kidsyoga.securework.co/top-videos/?video='+value[index]+'';
						  }  , 2000 );						
					}
				}
					 $.ajax({
