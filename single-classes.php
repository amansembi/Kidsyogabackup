...
<div class="container">
	<div class="singlpstttl">
		<p> <i class="fa fa-long-arrow-left" aria-hidden="true"></i> <a href="http://kidsyoga.securework.co/explore-classes">Back To All classes </a> </p>
		<?php
		...
		<?php global $post;
			...
			$videoID = get_post_meta( get_the_ID(), 'vimeo_videos_id', true );
			$vimeoVid = get_post_meta( $videoID, 'dgv_response', true );			
			$vd_id = preg_replace( '/\D/', '', html_entity_decode($vimeoVid) );
				if(isset($_GET['video'])){
					$vd_id = $_GET['video'];
...
<!--<div class="sglclsvid">
	<video width="100%" height="100%" controls autoplay  id="video-active">
		<source src="https://player.vimeo.com/video/<?php //echo vimeoVid[0]; ?>" type="video/mp4">>
	</video>
...
<div class="sglclsvid <?php if(empty($userid)){ echo 'openpoup'; }else{ echo ''; }  ?>">
...
<iframe src="https://player.vimeo.com/video/<?php echo $vd_id; ?>?autoplay=0&loop=0&autopause=0&badge=0&controls=0" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>	
			...
			<div class="col-md-4 sglvidfavlistcol">
				...
				$count = 1; 
				foreach($user_liked_vedio as $video){ 
				//var_dump($video);
...
// $('.sglclsvid .openpoup').on('click',function() {
              ...
              ld_video_players[element_id].on('timeupdate', function(something) {				  
				...
				if(autopalystatus1 === true){
					...
					if(idExist == true){
						index = index +1;	
							setTimeout( function(){ 
							 window.location = 'http://kidsyoga.securework.co/classes/be-a-yoga-bird/?video='+value[index]+'';
						  }  , 2000 );						
					}
				}
					 $.ajax({
